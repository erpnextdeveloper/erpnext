		...
		for wh in sorted(iwb_map[item]):
			for batch in sorted(iwb_map[item][wh]):
				...
				if qty_dict.opening_qty or qty_dict.in_qty or qty_dict.out_qty or qty_dict.bal_qty:
					data.append([item, item_map[item]["item_name"], item_map[item]["description"],getItemCategory(item), wh, batch,
						...
						flt(qty_dict.out_qty, float_precision), flt(qty_dict.bal_qty, float_precision),
						 item_map[item]["stock_uom"],frappe.db.get_value('Batch', batch, 'mfg_date'),frappe.db.get_value('Batch', batch, 'expiry_date'),frappe.db.get("Stock Ledger Entry",{"batch_no":batch},{"voucher_type":"Purchase Receipt"}).posting_date
					])
...
def get_columns(filters):
	...
	columns = [_("Item") + ":Link/Item:100"] + [_("Item Name") + "::150"] + [_("Description") + "::150"] +[_("Category") + ":Data:100"]+ \
	...
	[_("UOM") + "::90"] + [_("Mfg Date") + ":Date:90"] + [_("Expires On") + ":Date:90"]+ [_("Posting Date") + ":Date:90"]
...
def getItemCategory(item):
	data=frappe.db.sql("""select category from `tabItem` where item_code=%s""",item)
	if data:
		return data[0][0]
	else:
		return str()
