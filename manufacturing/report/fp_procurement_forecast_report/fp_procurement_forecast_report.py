...
def execute(filters=None):
	...
	for (company, item, warehouse) in sorted(iwb_map):
		...
		report_data = [item, item_map[item]["item_name"],
			item_map[item]["stock_uom"], getRequestQty(filters,item),qty_dict.opening_qty,getPOQty(filters,item),(flt(getRequestQty(filters,item))-((qty_dict.opening_qty)+(flt(getPOQty(filters,item))))),getClp(item)
		]
...
def get_columns():
	...
	columns = [
		...
		_("Item Name")+"::150",
		_("Stock UOM")+":Link/UOM:40",
		_("Requested Qty")+":Float:100",
		_("Opening Qty")+":Float:100",
		_("Purchase Order Qty")+":Float:100",
		_("To Be Ordered Qty")+":Float:100",
		_("CLP")+":Float:90"
	]
...
def getRequestQty(filters,item):
	f_date=filters.get("from_date")
	t_date=filters.get("to_date")
	temp1='FPREQ%'
	data=frappe.db.sql("""select sum(qty) from `tabMaterial Request Item` where item_code=%s and schedule_date between %s and %s and parent LIKE %s""",(item,f_date,t_date,temp1))
	if len(data):
		return data[0][0]
	else:
		return ""
...
def getClp(item):
	temp=0
	data=frappe.db.sql("""select clp from `tabItem` where name=%s""",item)
	if len(data):
		return data[0][0]
	else:
		return temp

def getPOQty(filters,item):
	f_date=filters.get("from_date")
	t_date=filters.get("to_date")
	data=frappe.db.sql("""select sum(qty) from `tabPurchase Order Item` where item_code=%s and schedule_date between %s and %s""",(item,f_date,t_date))
	if len(data):
		return data[0][0]
	else:
		return ""
