...
def execute(filters=None):
	...
	for (company, item, warehouse) in sorted(iwb_map):
		...
		report_data = [item, item_map[item]["item_name"],
			item_map[item]["brand"], warehouse,
			item_map[item]["stock_uom"], qty_dict.opening_qty,
			((qty_dict.opening_qty)*flt(getClp(item))), getRequestQty(filters,item),(flt(getRequestQty(filters,item))*flt(getClp(item))),
			qty_dict.in_qty,((qty_dict.in_qty)*flt(getClp(item))), 
			qty_dict.out_qty,((qty_dict.out_qty)*flt(getClp(item))), qty_dict.bal_qty,
			((qty_dict.bal_qty)*flt(getClp(item))), getClp(item)
		]
