...
def usertype():
	...
	if usertype[0][0]=="Regional Sales Manager" or usertype[0][0]=="Area Sales Manager":
		return "Sales Manager"
	
	elif usertype[0][0]=="Sales Officer":
		return "Sales Person"

	elif usertype[0][0]=="Technical Trainer" or usertype[0][0]=="Senior Technical Trainer":
...
def employeeLeaveApplicationList():
    data=frappe.db.sql("""select Permission.for_value,Leave1.employee_name,Leave1.leave_type,Leave1.from_date,Leave1.to_date,Leave1.leave_balance,Leave1.name,Leave1.description,Leave1.half_day,Leave1.half_day_slot from `tabUser Permission` as Permission INNER JOIN `tabLeave Application` as Leave1 on Permission.for_value=Leave1.employee where Permission.allow='Employee' and Permission.user=%s """,frappe.session.user)
    object_list_emp=[]
    for employee in data:
        d = collections.OrderedDict()
        d["emp"]=str(employee[0])
...
def employeeVisitList():
    data=frappe.db.sql("""select Permission.for_value,count(Permission.for_value) as count from `tabUser Permission` as Permission INNER JOIN `tabVisit` as Visit on Permission.for_value=Visit.sales_person where Permission.allow='Employee' and Permission.user=%s group by Permission.for_value """,frappe.session.user)
    object_list_emp=[]
    for employee in data:
        d = collections.OrderedDict()
        d["emp"]=str(employee[0])
...
# @frappe.whitelist()
# def employeeExpenseList(empid):
	
#     data=frappe.db.sql("""select Permission.for_value,Expense.employee_name,Leave1.leave_type,Leave1.from_date,Leave1.to_date,Leave1.leave_balance,Leave1.name,Leave1.description,Leave1.half_day,Leave1.half_day_slot from `tabUser Permission` as Permission INNER JOIN `tabExpense Claim` as Expense on Permission.for_value=Expense.employee where Permission.allow='Employee' and Permission.user=%s """,frappe.session.user)
#     object_list_emp=[]
#     for employee in data:
#         d = collections.OrderedDict()
#         d["emp"]=str(employee[0])
...
def employeeMonthlyExpenseList(empid):
	...
	for li in list:
		item = frappe.get_list("Expense Claim Detail", filters=[["Expense Claim Detail","parent","=",li.name]], fields=['expense_date','expense_type','description','claim_amount','sanctioned_amount'])
		li.expenses = item
	d = collections.OrderedDict()
	if len(list):
		d["msg"]="Data  Found"
...
@frappe.whitelist()
def SupplierWiseItemPrice(item_code):
	return frappe.get_all('Supplier Quotation Item', filters={'item_code': item_code}, fields=['parent', 'qty','stock_uom','rate'])
